...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    parent_category = serializers.CharField(
        source="category.parent.name", default=None, read_only=True
    )
    media = ProjectMediaSerializer(many=True, read_only=True)

    # Yeh logic frontend ko list bhejega: ["React", "Python"]
    ...
    def get_tech_list(self, obj):
        if obj.technologies:
            return [
                tech.strip() for tech in obj.technologies.split(",") if tech.strip()
            ]
        return []
...
class ProjectCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Project
        # Yahan 'technologies' add karna zaroori hai
        fields = [
            "title",
            "description",
            "category",

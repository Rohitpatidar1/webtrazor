...
def admin_login(request):
    ...
    if user and user.is_staff:
        ...
        return Response(
            {"token": token.key, "status": True, "message": "Login successful"}
        )
...
def projects(request):
    ...
    if category:
        qs = qs.filter(category__parent__name__icontains=category)
    if subcategory:
        qs = qs.filter(category__name__icontains=subcategory)
    if location:
...
def providers_list(request, service, location):
    ...
    data = ServiceProvider.objects.filter(
        service_type__icontains=clean_service, location__icontains=clean_location
    )
            ...
            return Response(
                {
                    ...
                    "data": ServiceRequestSerializer(
                        obj
                    ).data,  # Yeh client_name show karega
...
def update_project_with_media(request, project_id):
    try:
        ...
        if data.get("category"):
            project.category_id = data.get("category")

        # --- FIX: Technology Update Logic ---
        # Agar admin side se 'technologies' bhej rahe ho (comma separated string)
        if "technologies" in data:
            project.technologies = data.get("technologies")

        # Agar admin side se 'tech_list' bhej rahe ho (as an array/list)
        # Isko string mein convert karke save karenge taaki DB consistency bani rahe
        elif "tech_list" in data:
            tech_array = data.get("tech_list", [])
            if isinstance(tech_array, list):
                project.technologies = ", ".join(tech_array)
        # -------------------------------------
        ...
        # 2. Media Sync (Images)
        if "images" in data:
            # Sirf images delete karein, videos nahi
            project.media.filter(type="image").delete()
            new_images = data.get("images", [])
            for url in new_images:
                if url:  # check for empty strings
        ...
        if "video" in data:
            ...
            if new_video:
                ...
                ProjectMedia.objects.create(
                    project=project, file=new_video, type="video"
                )
            elif new_video == "":
                project.media.filter(type="video").delete()
        ...
        return Response(
            {
                "message": "Project, Technologies and Media updated successfully",
                "status": True,
            }
        )

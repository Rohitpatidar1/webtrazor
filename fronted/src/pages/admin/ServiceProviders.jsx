...
export default function ServiceProviders() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/add-provider/", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setFormData({ service_name: "", provider_name: "", contact_no: "", location: "", business_url: "" });
      } else {
  ...
  return (
    <div className={`min-h-screen p-6 md:p-10 transition-colors duration-500 ${
      ...
      <header className="mb-12">
        ...
        <p className="opacity-60 font-medium">Add and manage business service providers for the directory.</p>
      </header>

      <div className="grid lg:grid-cols-3 gap-8">
        
        {/* Form Section (Takes 2 columns) */}
        <div className={`lg:col-span-2 p-8 md:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden transition-all ${
          isDark ? "bg-[#0a2538] border border-white/5" : "bg-white border border-slate-100"
        }`}>
        ...
        <aside className="space-y-6">
          <div className={`p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden ${
            isDark ? "bg-[#0B3C5D]/30 border border-white/5" : "bg-[#0B3C5D] text-white"
          }`}>
            ...
            <ul className="space-y-6">
              {[
                { title: "Keywords Matter", desc: "Ensure Service Name matches what users search for." },
                { title: "Specific Location", desc: "Include area names like 'Vijay Nagar' for local SEO." },
                { title: "Direct Contact", desc: "Double-check the WhatsApp number for direct leads." }
              ].map((item, idx) => (
          ...
          {/* Decorative Card */}
          <div className={`p-6 rounded-[2.5rem] border border-dashed transition-all ${
            isDark ? "border-white/10 opacity-40 hover:opacity-100" : "border-slate-300 opacity-60 hover:opacity-100"
          }`}>

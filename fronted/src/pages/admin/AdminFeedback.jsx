...
import { Trash2, MessageSquare, Printer, Star, Calendar, Mail, Folder } from "lucide-react";
...
export default function AdminFeedback() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Bhai, kya aap sach mein ye feedback delete karna chahte hain?")) return;
    const token = localStorage.getItem("adminToken");

    try {
      const response = await fetch(`http://127.0.0.1:8000/api/admin/delete-feedback/${id}/`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className={`min-h-screen p-4 md:p-10 transition-colors duration-500 ${
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold opacity-50 tracking-[0.2em]">Total Records: {feedbacks.length}</p>
          </div>
        </div>
      ...
      {/* --- Table Container --- */}
      <div className={`rounded-[2.5rem] shadow-2xl overflow-hidden border transition-all duration-500 ${
        isDark ? "bg-[#0a2538] border-white/5 shadow-black/40" : "bg-white border-slate-200"
      }`}>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className={`${isDark ? "bg-white/5 text-white/50" : "bg-[#0b3c5d] text-white"} text-[11px] uppercase tracking-[0.2em] font-black`}>
            ...
            <tbody className={`divide-y transition-colors duration-500 ${isDark ? "divide-white/5" : "divide-slate-100"}`}>
              {loading ? (
                <tr><td colSpan="6" className="py-20 text-center font-bold italic animate-pulse text-[#00bf56]">Fetching feedbacks...</td></tr>
              ) : feedbacks.length === 0 ? (
                <tr><td colSpan="6" className="py-20 text-center opacity-30 italic">No feedback records found.</td></tr>
              ) : (
                feedbacks.map((item) => (
                  <tr key={item.id} className={`group transition-all duration-300 ${isDark ? "hover:bg-white/[0.02]" : "hover:bg-[#00bf56]/5"}`}>
                    {/* User Info */}
                    <td className="px-6 py-5">
                      <div className="font-black text-sm uppercase tracking-tight italic">{item.name}</div>
                      <div className="flex items-center gap-1 text-[10px] opacity-50 font-bold mt-1">
                        <Mail size={10} className="text-[#00bf56]" /> {item.email}
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-0.5">
                         {[...Array(5)].map((_, i) => (
                           <Star 
                            key={i} 
                            size={14} 
                            fill={i < (item.rating || 0) ? "#00bf56" : "transparent"} 
                            className={i < (item.rating || 0) ? "text-[#00bf56]" : "text-slate-300 opacity-30"} 
                           />
                         ))}
                      </div>
                    </td>
